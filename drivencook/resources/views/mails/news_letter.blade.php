...
<div class="container">
    ...
    @if (!empty($user['client_orders_count']) && $user['client_orders_count'] > 0)
        <p>
            Ce mois-ci tu as passé {{$user['client_orders_count']}} commande(s) chez DriveNCook !
        </p>
    @endif

    @if ($user['loyalty_point'] > 0 && !empty($fidelity))
        <p>
            Tu as à ce jour {{$user['loyalty_point']}} point de fidélités, ce qui te donnera droit jusqu'à {{$fidelity}}€ de
    ...
    @if (count($user['event_invited_30']) > 0 )
        <p>
            Tu as été invité à {{count($user['event_invited_30'])}} évènement(s) privé(s) ces trentes derniers jours !
        <ul>
