                            ...
                            <tbody>
                                @foreach($franchisee['purchase_order'] as $purchase_order)
                                    <tr id="order_{{$purchase_order['id']}}">
                                        ...
                                        <td>
                                            <button class="fa fa-edit"
                                                    onclick="onUpdateOrderModal({{ $purchase_order['id'] }}, '{{ $purchase_order['status'] }}',
                                                    '{{ DateTime::createFromFormat('Y-m-d',$purchase_order['date'])->format('d/m/Y') }}', '{{ $purchase_order['warehouse']['name'] }}', {{ $total }})"
                                                    data-toggle="modal"
...
@section('script')
    <script>
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (dataJ.response === "success") {
                            ...
                            alert(Lang.get('franchisee.stock_update_success'));
                            ...
                            alert(Lang.get('franchisee.ajax_error') + dataJ.message);
                        }
                    },
                    error: function (data) {
                        const dataJ = JSON.parse(data);
        ...
        function onRemoveStock(dish_id) {
            if (confirm(Lang.get('franchisee.stock_delete_confirm'))) {
                if (!isNaN(dish_id)) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == dish_id) {
                                alert(Lang.get('franchisee.stock_delete_success'));
                                document.getElementById("stock_" + dish_id).remove();
                            } else {
                                alert(Lang.get('franchisee.ajax_error') + data);
                            }
                        },
                        error: function (data) {
                            alert(Lang.get('franchisee.ajax_error') + data);
        ...
        function onUpdateOrderModal(id, current_status, date, warehouse_name, total) {
            document.getElementById('orderModalTitle').innerText = warehouse_name + ' - ' + date + ' - ' + total + " €";
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (dataJ.response === "success") {
                            ...
                            alert(Lang.get('franchisee.order_update_success'));
        ...
        function onRemoveOrder(id) {
            if (confirm(Lang.get('franchisee.order_delete_confirm'))) {
                if (!isNaN(id)) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == id) {
                                alert(Lang.get('franchisee.order_delete_success'));
